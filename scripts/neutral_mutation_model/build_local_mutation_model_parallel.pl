...
while (<IN>){
    ...
    # mutations in neutral regions. This subcommand returns a file with 12 columns:
    ## 1)chrom
    ## 2)start
    ## 3)end
    ...
    my $cmd = qq[tabix $mutation_file $chr:$start-$end | perl filter_mutation.pl $min_coverage $freq_cutoff | perl add_context_mutation_rate.pl $context_frequency_table | bedtools intersect -a - -b $covariate_file -sorted -wo | cut -f1-7,11-12 | intersectBed -loj -a - -b $neutral_file -sorted ];
